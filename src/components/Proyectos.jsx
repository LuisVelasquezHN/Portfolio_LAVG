...
const TagIcon = ({ type }) => {
  switch (type) {
    ...
    case 'tailwind':
      return (
        <svg viewBox="0 0 128 128" width="15" height="15">
          <path d="M64.004 25.602c-17.067 0-27.73 8.53-32 25.597 6.398-8.531 13.867-11.73 22.398-9.597 4.871 1.214 8.352 4.746 12.207 8.66C72.883 56.629 80.145 64 96.004 64c17.066 0 27.73-8.531 32-25.602-6.399 8.536-13.867 11.735-22.399 9.602-4.87-1.215-8.347-4.746-12.207-8.66-6.27-6.367-13.53-13.738-29.394-13.738zM32.004 64c-17.066 0-27.73 8.531-32 25.602C6.402 81.066 13.87 77.867 22.402 80c4.871 1.215 8.352 4.746 12.207 8.66 6.274 6.367 13.536 13.738 29.395 13.738 17.066 0 27.73-8.53 32-25.597-6.399 8.531-13.867 11.73-22.399 9.597-4.87-1.214-8.347-4.746-12.207-8.66C55.128 71.371 47.868 64 32.004 64z" fill="#38bdf8"></path>
        </svg>
      );
    case 'sass':
      return (
        <svg viewBox="0 0 512 384" xmlns="http://www.w3.org/2000/svg" width="16" height="16">
    ...
    case 'node':
      return (
        <svg viewBox="0 0 256 292" xmlns="http://www.w3.org/2000/svg" width="16" height="16">
          <path fill="#6CC04A" d="M134.923 1.832c-4.344-2.443-9.502-2.443-13.846 0L6.787 67.801C2.443 70.244 0 74.859 0 79.745v132.208c0 4.887 2.715 9.502 6.787 11.945l114.29 65.968c4.344 2.444 9.502 2.444 13.846 0l114.29-65.968c4.344-2.443 6.787-7.058 6.787-11.945V79.745c0-4.886-2.715-9.501-6.787-11.944L134.923 1.832Z" />
        </svg>
      );
    case 'php':
      return (
...
const Tag = ({ tag }) => {
  ...
  return (
    <span className={`flex flex-row gap-2 items-center justify-center text-[11px] p-1 rounded-2xl w-[80px] ${getColors(tag.type)}`}>
...
export const Proyectos = () => {
  ...
  return (
    <>
        ...
        <div ref={stackAreaRef} id="projects" className="w-full h-[400vh] relative flex flex-col-reverse sm:flex-row hiddenObs">
          <div className="h-screen flex-1 sticky top-0 flex flex-col justify-center px-10">
            ...
            <p className="w-[420px] text-2xl font-semibold mt-6 dark:text-[#4484bf] text-[#0686ff]">
              {activeProject?.titleKey ? t(activeProject.titleKey) : activeProject?.title}
            </p>
            <div className="tech flex flex-row gap-2 items-center mt-2">
              {activeProject?.tags.map((tag, idx) => (
                <Tag key={idx} tag={tag} />
              ))}
            </div>

            <p className="max-w-[450px] text-[16px] mt-4 dark:text-[#e2e0e0] text-[#515151] text-pretty">
          ...
          <div className="h-screen flex-1 sticky top-0 relative overflow-hidden">
            {projectsData.map((project, index) => (
              <div
                ...
                className="w-[340px] h-[340px] rounded-[25px] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 shadow-2xl bg-cover bg-center"
      ...
      {isMobile && (
        <div id="projects" className="w-full flex flex-col p-6">
          <h1 className="flex items-center mb-8 text-3xl font-semibold gap-x-3 text-black/80 dark:text-white">
          ...
          {projectsData.map((project, index) => (
            <div key={index} className="flex flex-col mb-12 last:mb-0">
              ...
              <p className="text-sm mt-4 dark:text-[#e2e0e0] text-[#515151]">
